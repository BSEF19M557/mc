        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            if(session!=null){
                if(session.getAttribute("type")==null){
                    ...
                    if("patient".equals(session.getAttribute("type").toString())){
                    ...
                    else if(request.getParameter("Rem").equals("")){
                        String value="";
                       ...
                       String query;
                       query="update info set Remarks='"+request.getParameter("Rem")+"' where UserName='"+request.getParameter("uName")+"' ";
                        update.executeUpdate(query);
                        query="update info set Doctor='' where UserName='"+request.getParameter("uName")+"' ";
                        update.executeUpdate(query);
                        String value="";
                        request.setAttribute("remarked",value);
                        RequestDispatcher rd2 = request.getRequestDispatcher("patientForDoc.jsp");
                        rd2.forward(request,response);
                       //JDBC Connectity
                       con.close();
